		...
		this.processNews = function (msg) {
			...
			if (!event.backfill) {
				if (event.data.type === 'post') {
					var item = $('<div>');
					var endpoint = proxyEndPoint(event.data.about);
					...
					$('body').trigger('NotifyLiveElement', [event.data.type, event.data.about, proxyEndPoint(event.data.about + '/comment/' + event.data.uuid)]);
				}
				else if (event.data.type === 'react') {
					$('body').trigger('NotifyLiveElement', [event.data.type, event.data.about, proxyEndPoint(event.data.about + '/reactions')]);
				}
